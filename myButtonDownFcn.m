...
% 2d tuning
D.variable.tuning_2d = squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  H.cp(1,2), H.cp(1,1))));
set(H.tuning_2d, 'CData', D.variable.tuning_2d);

% first feature tuning
D.variable.tuning_F1 = mean(squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  H.cp(1,2), H.cp(1,1)))), 1);
...
% second feature tuning
D.variable.tuning_F2 = mean(squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  H.cp(1,2), H.cp(1,1)))), 2);
