...
save([datapath, '\DataMat_102D_NatVoc_12reps.mat'], 'DataMat', 'para', '-v7.3')
...
%% generage a mask for cross-validation
M = double(rand(size(X)) > 0.1);
X_mask = X.*M;
...
%% get response ranking based on applied mask
R_core = mean(X,2);
