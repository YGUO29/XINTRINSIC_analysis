...
corr_matrix(:,k) = diag(corr(D_test_rep2_denoised, D_test_rep1));
...
upper_bound = diag(corr(D_test_rep2_denoised, D_test_rep1_denoised));
figurex;
subplot(1,2,1)
...
legend({'denoise one split', 'denoise two splits', 'y=x', 'y=sqrt(x)'})
%% denoise using R
...
%     title(['Original, ', F.sound_names{i}])
