...
D.variable.trace = squeeze(D.DataMat_norm(:, floor(D.para.height/2), floor(D.para.width/2), :))';
...
D.variable.tuning_2d = squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  ...
    floor(D.para.height/2), floor(D.para.width/2))));
% H.tuning_2d = imagesc(flipud(D.variable.tuning_2d), 'XData', D.F1, 'YData', D.F2);
H.tuning_2d = imagesc(D.variable.tuning_2d, 'XData', D.F1, 'YData', D.F2);
set(gca, 'YDir', 'normal')
...
D.variable.tuning_F1 = mean(squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  ...
    floor(D.para.height/2), floor(D.para.width/2)))), 1);
if isfield(D,'F1_log')
...
D.variable.tuning_F2 = mean(squeeze(D.RMap(:,:,sub2ind([D.para.height, D.para.width],  ...
    floor(D.para.height/2), floor(D.para.width/2)))), 2);
